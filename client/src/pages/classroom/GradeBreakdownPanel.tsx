...
export default function GradeBreakdownPanel({
  ...
  const visibleItems = breakdown.breakdown.filter((b) => b.status !== "no-items");
  if (visibleItems.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground text-center">
        No graded items yet.
      </div>
    );
  }

  const allPending = visibleItems.every((b) => b.status === "pending");
  if (allPending) {
    return (
      <div className="rounded-2xl border border-dashed border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground text-center">
        No grades yet — assignments are pending.
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-foreground">
          Overall Grade
          {breakdown.isPartial && (
            <span className="ml-2 text-xs font-normal text-amber-600">(partial — some types pending)</span>
          )}
        </span>
          ...
          return (
            <div key={item.type} className="flex items-center gap-2.5">
              ...
              {item.status === "zero-weight" && (
                <span className="text-xs text-muted-foreground italic">Not counted</span>
              )}
              {item.status === "graded" && item.average !== null && (
                <div className="flex items-center gap-1.5 flex-1 min-w-0">
                  ...
                  <span className={`text-xs font-semibold tabular-nums shrink-0 ${meta.text}`}>
                    {item.average}%
                  </span>
      ...
      {breakdown.pendingTypes.length > 0 && !allPending && (
        <div className="flex items-start gap-1.5 pt-0.5">
          <Info className="h-3.5 w-3.5 text-amber-500 shrink-0 mt-0.5" />
          <p className="text-xs text-amber-600">
            {breakdown.pendingTypes.join(", ")} pending — overall will update when graded.
          </p>
        </div>
      )}
    </div>
