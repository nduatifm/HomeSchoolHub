...
export async function apiRequest(url: string, options: RequestInit = {}) {
  // All sessions (including impersonation) are managed server-side via httpOnly cookies.
  // No Authorization header or localStorage token is ever needed.
...
export async function apiUpload(url: string, formData: FormData, options: RequestInit = {}) {
  // All sessions are managed server-side via httpOnly cookies — no Authorization header needed.
  const response = await fetch(url, {
    method: 'POST',
    ...options,
    body: formData,
