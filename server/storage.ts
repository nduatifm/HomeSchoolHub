...
class PrismaStorage implements IStorage {
  ...
  async getStudentById(id: number): Promise<Student | null> {
    return await prisma.student.findUnique({ where: { id } }) as Student | null;
  }
  ...
  async getAssignmentById(id: number): Promise<Assignment | null> {
    return await prisma.assignment.findUnique({ where: { id } }) as Assignment | null;
  }

  async getAssignmentsByTeacher(teacherId: number): Promise<Assignment[]> {
    return await prisma.assignment.findMany({ where: { teacherId } }) as Assignment[];
  }
  ...
  async createStudentAssignment(studentAssignment: InsertStudentAssignment): Promise<StudentAssignment> {
    return await prisma.studentAssignment.create({ data: studentAssignment }) as StudentAssignment;
  }

  async getStudentAssignmentById(id: number): Promise<StudentAssignment | null> {
    return await prisma.studentAssignment.findUnique({ where: { id } }) as StudentAssignment | null;
  }

  async getStudentAssignmentsByStudent(studentId: number): Promise<StudentAssignment[]> {
    return await prisma.studentAssignment.findMany({ where: { studentId } }) as StudentAssignment[];
  }
  ...
  async getMaterialsByTeacher(teacherId: number): Promise<Material[]> {
    return await prisma.material.findMany({ where: { teacherId } }) as Material[];
  }

  async getMaterialsBySubject(subject: string): Promise<Material[]> {
    return await prisma.material.findMany({ where: { subject } }) as Material[];
  }
  ...
  async getSchedulesByTeacher(teacherId: number): Promise<Schedule[]> {
    return await prisma.schedule.findMany({ where: { teacherId } }) as Schedule[];
  }
  ...
  async getFeedbackByStudent(studentId: number): Promise<Feedback[]> {
    return await prisma.feedback.findMany({ where: { studentId } }) as Feedback[];
  }

  async getFeedbackByTeacher(teacherId: number): Promise<Feedback[]> {
    return await prisma.feedback.findMany({ where: { teacherId } }) as Feedback[];
  }
  ...
  async getAttendanceByStudent(studentId: number): Promise<Attendance[]> {
    return await prisma.attendance.findMany({ where: { studentId } }) as Attendance[];
  }
  ...
  async getPaymentsByParent(parentId: number): Promise<Payment[]> {
    return await prisma.payment.findMany({ where: { parentId } }) as Payment[];
  }
  ...
  async createTutorRequest(request: InsertTutorRequest): Promise<TutorRequest> {
    return await prisma.tutorRequest.create({ data: request }) as TutorRequest;
  }

  async getTutorRequestById(id: number): Promise<TutorRequest | null> {
    return await prisma.tutorRequest.findUnique({ where: { id } }) as TutorRequest | null;
  }

  async getTutorRequestsByParent(parentId: number): Promise<TutorRequest[]> {
    return await prisma.tutorRequest.findMany({ where: { parentId } }) as TutorRequest[];
  }
  ...
  async createProgressReport(report: InsertProgressReport): Promise<ProgressReport> {
    return await prisma.progressReport.create({ data: report }) as ProgressReport;
  }

  async getProgressReportsByStudent(studentId: number): Promise<ProgressReport[]> {
    return await prisma.progressReport.findMany({ where: { studentId } }) as ProgressReport[];
  }

  async getProgressReportsByTeacher(teacherId: number): Promise<ProgressReport[]> {
    return await prisma.progressReport.findMany({ where: { teacherId } }) as ProgressReport[];
  }

  async createClarification(clarification: InsertClarification): Promise<Clarification> {
    return await prisma.clarification.create({ data: clarification }) as Clarification;
  }

  async getClarificationsByStudent(studentId: number): Promise<Clarification[]> {
    return await prisma.clarification.findMany({ where: { studentId } }) as Clarification[];
  }
  ...
  async createParentalControl(control: InsertParentalControl): Promise<ParentalControl> {
    return await prisma.parentalControl.create({ data: control }) as ParentalControl;
  }
  ...
  async getRatingsByTeacher(teacherId: number): Promise<TutorRating[]> {
    return await prisma.tutorRating.findMany({ where: { teacherId } }) as TutorRating[];
  }

  async getRatingsByParent(parentId: number): Promise<TutorRating[]> {
    return await prisma.tutorRating.findMany({ where: { parentId } }) as TutorRating[];
  }
  ...
  async getStudentInviteByToken(token: string): Promise<StudentInvite | null> {
    return await prisma.studentInvite.findUnique({ where: { token } }) as StudentInvite | null;
  }
